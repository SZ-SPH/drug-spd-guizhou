...
<template>
	<div>
		...
		<el-row :gutter="15" class="mb10">
			<el-col :span="1.5">
				<el-button type="primary" v-hasPermi="['ChesView:add']" plain icon="plus" @click="CodehandleAdd">
					{{ $t('btn.add') }}
				</el-button>
			</el-col>
			<el-col :span="1.5">
			...
			<el-col :span="1.5">
				<el-button type="danger" :disabled="Codemultiple" v-hasPermi="['ChesView:delete']" plain icon="delete"
					@click="CodehandleDelete">
					{{ $t('btn.delete') }}
				</el-button>
			</el-col>
			<el-col :span="1.5">
				<el-button type="danger" v-hasPermi="['ChesView:delete']" plain icon="delete" @click="CodehandleClear">
					{{ $t('btn.clean') }}
				</el-button>
			</el-col>
			<el-col :span="1.5">
				<el-dropdown trigger="click" v-hasPermi="['ChesView:import']">
					...
					<template #dropdown>
						<el-dropdown-menu>
							<el-dropdown-item command="upload">
								<importData templateUrl="business/ChesView/importTemplate"
									importUrl="/business/ChesView/importData" @success="CodehandleFileSuccess">
								</importData>
							</el-dropdown-item>
						</el-dropdown-menu>
					</template>
		...
		<el-table @row-dblclick="TaskDbList" :data="CodedataList" v-loading="Codeloading" ref="table" border
			...
			<el-table-column prop="physicName" label="平台药品名称" align="center" :show-overflow-tooltip="true"
			...
			<el-table-column prop="drugEntBaseInfoId" label="平台药品id" align="center" :show-overflow-tooltip="true"
		...
		<el-table :data="tabe2Data" v-loading="Codeloading" ref="table" border
			...
			<el-table-column prop="drugMnemonicCode" label="作业跟踪" align="center" :show-overflow-tooltip="true">
				<!-- <template #default="scope">
					<div class="action">
						<div class="state"
							:class="scope.row.state == 0 ? 'bg-info' : scope.row.state == 1 ? 'bg-success' : 'bg-error'">
						</div>
		...
		<el-dialog :title="Codetitle" :lock-scroll="false" v-model="Codeopen">
			<el-form ref="CodeformRef" :model="Codeform" :rules="Coderules" label-width="100px">
				<el-row :gutter="20">
					...
					<el-col :lg="12">
						<el-form-item label="药品id" prop="exprie">
							<el-input v-model="Codeform.exprie" placeholder="请输入药品id" />
						</el-form-item>
					</el-col>
...
import {
	listChesView,
	addChesView, delChesView,
	updateChesView, getChesView,
	clearChesView,
}
	from '@/api/business/ChesView.js'
...
const Codecolumns = ref([
	...
	{ visible: false, align: 'center', type: '', prop: 'physicName', label: '平台药品名称', showOverflowTooltip: true },
	{ visible: false, align: 'center', type: '', prop: 'exprie', label: '有效期', showOverflowTooltip: true },
	{ visible: false, align: 'center', type: '', prop: 'drugEntBaseInfoId', label: '平台药品id', showOverflowTooltip: true },
...
const tabe2Data = ref([{
	id: 1,
	drugName: "管理员",
	date: "2024-01-31 15:52:30",
	drugMnemonicCode: "已完成",
	detail: "基础测试供应商发送货清单，送货单号SH00013213",
},
{
	id: 2,
	drugName: "管理员",
	date: "2024-2-03 10:03:26",
	drugMnemonicCode: "已完成",
	detail: "基础测试供应商送货单入库，入库单号SH00013213",
},
{
	id: 3,
	drugName: "管理员",
	date: "2024-2-03 10:03:26",
	drugMnemonicCode: "未完成",
	detail: "医嘱已出单使用，医嘱单号SH00013213",
}
])
var CodedictParams = [
]
...
function CodegetList() {
	...
	listChesView(CodequeryParams).then(res => {
	...
	proxy.$refs["CodeformRef"].validate((valid) => {
		if (valid) {
			...
			if (form.value.id != undefined && Codeopertype.value === 2) {
				updateChesView(form.value).then((res) => {
					proxy.$modal.msgSuccess("修改成功")
					Codeopen.value = false
					CodegetList()
				})
			} else {
				addChesView(Codeform.value).then((res) => {
...
function CodehandleExport() {
	proxy
		...
		.then(async () => {
			await proxy.downFile('/business/ChesView/export', { ...CodequeryParams })
		})
}
